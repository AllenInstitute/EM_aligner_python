...
PetscErrorCode ReadVec (MPI_Comm COMM, PetscViewer viewer, char *varname,
			Vec * newvec, PetscInt * n);

PetscErrorCode ReadVecWithSizes (MPI_Comm COMM, PetscViewer viewer,
				 char *varname, Vec * newvec, PetscInt * n,
				 PetscInt nlocal, PetscInt nglobal,
				 PetscBool trunc);

PetscErrorCode ReadIndexSet (MPI_Comm COMM, PetscViewer viewer, char *varname,
			     IS * newIS, PetscInt * n);

PetscErrorCode ShowMatInfo (MPI_Comm COMM, Mat * m, const char *mesg);

void GetGlobalLocalCounts (int nfiles, PetscInt ** metadata,
			   int local_firstfile, int local_lastfile,
			   PetscInt * global_nrow, PetscInt * global_ncol,
			   PetscInt * global_nnz, PetscInt * local_nrow,
